...
from data import Constants, APICourierUrls, APICourierResponseTexts
...
class TestCreatingCourier:
    ...
    def test_creating_courier_courier_already_exists_error_message(self, reg_new_courier_return_login_pass_del_this_courier):
        ...
        assert response.status_code == 409 and response.json()["message"] == APICourierResponseTexts.CREATING_COURIER_ALREADY_EXISTS_ERROR
    ...
    def test_creating_courier_no_required_field_error_message(self, login, password, first_name):
        ...
        assert response.status_code == 400 and response.json()["message"] == APICourierResponseTexts.CREATING_COURIER_NO_REQUIRED_FIELD_ERROR
...
class TestCourierLogin:
    ...
    def test_courier_login_wrong_login_pass_error_message(self, reg_new_courier_return_login_pass_del_this_courier):
        login_pass = reg_new_courier_return_login_pass_del_this_courier[1]
        payload = {"login": f'{login_pass[0]+'test'}', "password": f'{login_pass[1]+'12345'}'}
        response = requests.post(Constants.MAIN_URL + APICourierUrls.COURIER_LOGIN_URL, data=payload)
        assert response.status_code == 404 and response.json()["message"] == APICourierResponseTexts.COURIER_LOGIN_WRONG_LOGIN_PASS_ERROR
    ...
    def test_courier_login_no_login_pass_error_message(self, login, password):
        ...
        response = requests.post(Constants.MAIN_URL + APICourierUrls.COURIER_LOGIN_URL, data=payload)
        assert response.status_code == 400 and response.json()["message"] == APICourierResponseTexts.COURIER_LOGIN_NO_LOGIN_PASS_ERROR
...
class TestDeleteCourier:
    ...
    def test_delete_courier_no_id_error_message(self):
        ...
        assert response.status_code == 400 and response.json()["message"] == APICourierResponseTexts.DELETE_COURIER_NO_ID_ERROR
    ...
    def test_delete_courier_non_existent_id_error_message(self, reg_new_courier_return_login_pass_del_this_courier):
        ...
        response = requests.delete(Constants.MAIN_URL + APICourierUrls.DELETE_COURIER_URL + str(courier_id))
        assert response.status_code == 404 and response.json()["message"] == APICourierResponseTexts.DELETE_COURIER_NON_EXISTENT_ID_ERROR
